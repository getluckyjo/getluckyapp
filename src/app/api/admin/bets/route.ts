...
export async function GET(request: Request) {
  ...
  try {
    let query = auth.adminClient.from('bets').select(BET_SELECT, { count: 'exact' })
    ...
    const names = await namesForBets(auth.adminClient, rows)
    let records: AdminBetRecord[] = rows.map(b => toAdminBetRecord(b, names))

    // Name search is a post-filter on the page (Batch 6 moves it into SQL).
    if (search) {
      const s = search.toLowerCase()
      records = records.filter(b =>
        (b.userName?.toLowerCase() ?? '').includes(s) || b.courseName.toLowerCase().includes(s) || b.id.toLowerCase().includes(s),
      )
    }
